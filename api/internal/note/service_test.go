...
func (f *fakeStore) Create(_ context.Context, content string, _ []float32) (note.Note, error) {
	f.created = append(f.created, content)
	return note.Note{ID: "id-1", Content: content, CreatedAt: time.Now()}, nil
}

func (f *fakeStore) Search(_ context.Context, _ []float32, limit int) ([]note.SearchHit, error) {
	f.searchLimit = limit
	return nil, nil
}

func (f *fakeStore) List(_ context.Context, limit, offset int) ([]note.Note, error) {
...
func TestCreateTrimsContentAndUsesDocumentInputType(t *testing.T) {
	...
	created, err := svc.Create(context.Background(), "  今天想到的點子  ")
	if err != nil {
...
func TestCreateRejectsEmptyAndOverlongContent(t *testing.T) {
	...
	if _, err := svc.Create(context.Background(), tooLong); !errors.Is(err, note.ErrContentTooLong) {
...
func TestCreateSurfacesEmbeddingFailure(t *testing.T) {
	...
	if _, err := svc.Create(context.Background(), "內容"); err == nil {
...
func TestSearchUsesQueryInputTypeAndClampsLimit(t *testing.T) {
	...
	if _, err := svc.Search(context.Background(), "點子", 0); err != nil {
		t.Fatalf("預期成功，卻得到 %v", err)
	}
	...
	if _, err := svc.Search(context.Background(), "點子", 9999); err != nil {
		t.Fatalf("預期成功，卻得到 %v", err)
	}
...
func TestSearchRejectsEmptyQuery(t *testing.T) {
	svc, _, _ := newService()
	if _, err := svc.Search(context.Background(), "  ", 5); !errors.Is(err, note.ErrEmptyQuery) {
...
func TestListClampsLimitAndOffset(t *testing.T) {
	...
	if _, err := svc.List(context.Background(), 0, -5); err != nil {
		t.Fatalf("預期成功，卻得到 %v", err)
	}
...
func TestSearchFiltersOutLowSimilarityHits(t *testing.T) {
	...
	hits, err := svc.Search(context.Background(), "向量索引", 10)
	if err != nil {
		t.Fatalf("預期成功，卻得到 %v", err)
	}
	if len(hits) != 2 {
...
func (s *scoredStore) Search(_ context.Context, _ []float32, _ int) ([]note.SearchHit, error) {
