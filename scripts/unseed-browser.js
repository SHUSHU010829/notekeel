// 移除範例測資 —— 在「已登入的隨手記網頁」上打開瀏覽器主控台（F12 → Console），
// 整段貼上後按 Enter。
//
// 只會刪掉「內容與範例測資完全相同」的筆記，你自己寫的不會被動到。
// 刪除不需要呼叫 Voyage，所以不受 3 RPM 限制。
//
...
;(async () => {
  ...
  const { notes } = await (await fetch('/api/notes?limit=200')).json()
  const targets = (notes ?? []).filter((note) => seeded.has(note.content))

  if (targets.length === 0) {
    console.log('沒有找到範例測資，不需要清理。')
    return
  }
  console.log(`找到 ${targets.length} 則範例測資，開始刪除…`)

  let removed = 0
  for (const note of targets) {
    const response = await fetch(`/api/notes/${note.id}`, { method: 'DELETE' })
    if (!response.ok) {
      console.error(`✗ ${response.status}`, note.content.slice(0, 20), await response.text())
      continue
    }
    removed += 1
  }

  console.log(`完成：刪除 ${removed}/${targets.length} 則，重新整理頁面就看得到。`)
})()
