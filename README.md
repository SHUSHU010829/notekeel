...
- **換模型不用改程式**：部分模型（例如 Haiku 4.5）不支援 `effort` 參數，
  遇到就自動改成不帶 effort 重送一次，並記住這個模型不支援。
- 標籤失敗（金鑰錯、模型拒絕）只會寫進 log，不影響筆記本身；
  手動觸發的 `/api/notes/tag` 會把原因放在回應的 `detail`。
...
同一組查詢，只開向量搜尋 vs 加上 rerank（voyage-4-lite + rerank-2.5-lite）：

| 查詢 → 該找到的筆記 | 純向量 | 加 rerank |
| --- | --- | --- |
| 節稅 → 報稅收據 | 第 1 名（0.402） | 第 1 名（0.396） |
| 腰痛 → 深蹲練核心 | 第 1 名（0.338） | 第 1 名（0.484） |
| 想存錢 → 緊急預備金 | 第 **2** 名（0.182） | 第 1 名（0.426） |
| 怎麼沖咖啡不會苦 → 淺焙水溫 | 第 1 名（0.598） | 第 1 名（0.520） |
| 向量資料庫怎麼建索引 → pgvector HNSW | 第 1 名（0.560） | 第 1 名（0.605） |
| 量子力學的測不準原理（無關） | 最高分 0.309 | 最高分 0.309 |

| | 純向量 | 加 rerank |
| --- | --- | --- |
| 正確命中的最低分 | 0.182 | **0.396** |
| 無關查詢的最高分 | 0.309 | 0.309 |
| 兩者是否分得開 | ✗ 重疊 | ✓ 分開 |

結論與注意事項：

1. **cosine 分數切不乾淨**：正確命中可能只有 0.18，比無關查詢的 0.309 還低，
   所以 `SEARCH_MIN_SIMILARITY` 維持 `0`，過濾交給 rerank。
2. **rerank 的分數才適合設門檻**：這組資料的建議值是 `RERANK_MIN_SCORE=0.3`～`0.35`。
   但樣本只有六組、間距只有 0.09，寧可設低一點 —— 漏掉自己記過的筆記，
   比多看到一則弱相關更難受，而且介面本來就會顯示相關度百分比讓你自己判斷。
3. **絕對分數不是百分比**：0.4 已經是不錯的命中，別拿 0.7 之類的直覺值當門檻。
4. **查詢越完整分數越高**：純向量時兩個字的「想存錢」只有 0.18，
